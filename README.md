...
5. 透過GCP->控制台->Comuute Engine，可以依之前所述方式重新開啟瀏覽器終端機，輸入指令１，可以列出目前存在的視窗，下圖可以看到之前建立的視窗gd01togd02。輸入指令２可以連回gd01togd02視窗。當我們關閉終端機視窗，下一次透過GCP->控制台->Comuute Engine，重新進入VM時，就可以透過指令2，連回之前開啟的視窗查詢執行結果。
輸入指令1：screen -ls
